...
async def test_setup_does_not_connect(
    ...
    """Nothing is read at setup: the lock hands that out when it advertises awake."""
    mock_ttlock_connection.async_query_state.assert_not_awaited()
    mock_ttlock_connection.async_start.assert_not_awaited()


async def test_the_coordinator_has_no_polling_interval(hass, setup_integration) -> None:
    assert setup_integration.runtime_data.coordinator.update_interval is None


async def test_permanent_connection_starts_the_session(
    hass,
    sample_stored_key,
    enable_bluetooth,
    enable_custom_integrations,
